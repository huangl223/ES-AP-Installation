note
	...
	date: "$Date: 2021-04-06 13:22:01 +0300 (Вт, 06 апр 2021) $"
	revision: "$Revision: 105267 $"

class FTP_PROTOCOL inherit

	NETWORK_RESOURCE
		redefine
			address, is_resource_open, put, read, reuse_connection, make
		end
		...
		do
			if is_proxy_used then
				check attached proxy_connection as l_proxy then
					Result := l_proxy.is_resource_open
				end
			else
