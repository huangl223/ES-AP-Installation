note
	...
	date: "$Date: 2020-05-19 17:40:37 +0300 (Вт, 19 май 2020) $"
	revision: "$Revision: 104267 $"
...
feature -- Status setting
	...
	open
		...
		deferred
			...
			failure_means_error: not is_resource_open implies error
		end

	close
			-- Close.
		require
			open: is_resource_open
		deferred
		ensure
			closed: not is_resource_open
			mode_unchanged: mode = old mode
			no_packet_available: not (has_packet and is_packet_pending)
		end
	...
	reuse_connection (other: DATA_RESOURCE)
		...
		require
			...
			other_opened: other.is_resource_open
		deferred
		end
...
feature -- Removal
	...
	dispose
			-- Clean up resource.
		do
			if is_resource_open then close end
		end
...
feature -- Output
	...
	put (other: DATA_RESOURCE)
		...
		require
			...
			resources_open: is_resource_open and other.is_resource_open
...
invariant
	...
	pending_constraint: is_packet_pending implies
						(is_resource_open and is_readable and transfer_initiated)
